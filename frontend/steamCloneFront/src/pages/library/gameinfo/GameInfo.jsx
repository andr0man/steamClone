...
const FALLBACK_COVER =
  "https://via.placeholder.com/300x400/1A2838/66c0f4?text=Game";
...
const mapGame = (resp) => {
  ...
  const genres = Array.isArray(g?.genres)
    ? g.genres.map((x) => x?.name ?? x).filter(Boolean)
    : [];
  const tags = Array.isArray(g?.tags)
    ? g.tags.map((x) => x?.name ?? x).filter(Boolean)
    : [];
...
const GameInfo = () => {
  ...
  const { data, isFetching, isError, error, refetch } = useGetGameByIdQuery(
    gameId,
    { skip: !gameId }
  );
  const { data: inLibData, refetch: refetchInLib } = useGetIsInGameLibraryQuery(
    gameId,
    { skip: !gameId }
  );
  ...
  if (!gameId) {
    return (
      <div className="gameinfo-error-state">
        ...
        <button
          onClick={() => navigate(-1)}
          className="action-button-gi back-button-gi"
        >
  ...
  return (
    <div className="gameinfo-page-wrapper">
      ...
      <header
        ...
        style={{
          backgroundImage: `linear-gradient(to top, rgba(21,26,33,1) 10%, rgba(21,26,33,0.7) 50%, rgba(21,26,33,0.3) 80%, transparent 100%), url(${
            headerImageUrl || FALLBACK_HEADER
          })`,
        }}
      >
        <div className="gameinfo-header-content">
          ...
          <div className="gameinfo-title-meta">
            ...
            <div className="gameinfo-quick-details">
              ...
              {releaseDate && (
                <span>
                  <strong>Released:</strong> {fmtDate(releaseDate)}
                </span>
              )}
            </div>
          </div>
          <div className="gameinfo-actions-header">
            {isOwned ? (
              ...
              <button
                className="action-button-gi store-link"
                disabled={buying}
                onClick={handleBuy}
              >
            ...
            {!isOwned && (
              <Link
                to={`/store/game/${id}`}
                className="action-button-gi secondary"
              >
                View Store Page
              </Link>
            )}
      ...
      <main className="gameinfo-main-content-area">
        <div className="gameinfo-left-column">
            ...
            <div className="media-tabs">
              <button
                className={`media-tab-button ${
                  activeMediaTab === "screenshots" ? "active" : ""
                }`}
          ...
          <section className="gameinfo-section description-section">
            <h2>
              <InfoIcon size={20} /> About This Game
            </h2>
            <div
        ...
        <aside className="gameinfo-right-sidebar">
          <section className="gameinfo-section details-list">
            ...
            <ul>
              ...
              <li>
                <strong>Developer:</strong> {developer || "N/A"}
              </li>
              <li>
                <strong>Publisher:</strong> {publisher || "N/A"}
              </li>
              <li>
                <strong>Release Date:</strong> {fmtDate(releaseDate)}
              </li>
            </ul>
          </section>
